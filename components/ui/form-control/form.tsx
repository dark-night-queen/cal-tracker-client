...
export const RadioElement = ({ options, direction = 'vertical', ...props }: IRadioElementProps) => {
  return (
    <RadioGroup className={`gap-3 ${direction == 'horizontal' ? 'flex-row' : ''}`} {...props}>
      {options.map((option) => (
        <Radio key={option} value={option} size="sm" isInvalid={false} isDisabled={false}>
          <RadioIndicator>
